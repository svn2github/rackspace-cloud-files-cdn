...
$rs_cdn(document).ready(function() {
	...
	$rs_cdn('#synchronize').click(function(e) {
		...
		$rs_cdn('#file_upload').html('<br/><strong>Starting sync...</strong>');
				...
				if (response != null) {
					...
					for(var key in response) {
					    if(response.hasOwnProperty(key)) {
					        first_file = response[key].file_name;
					        break;
					    }
					}
					...
					$rs_cdn.each(response, function(key, value) {
						var file_name = value.file_name.substring(value.file_name.lastIndexOf('/')+1);
						var file_path = encodeURIComponent(value.file_name);
						rs_cdn_queue_length++;
						rs_cdn_queue.add({
							url: ajaxurl+'?action=sync_existing_file&file_path='+file_path,
